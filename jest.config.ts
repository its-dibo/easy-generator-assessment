...
import { readSync } from "@impactor/nodejs"
import { type IPackage } from "@impactor/nx-manager"
...
export default function jestConfig(config?: Options): JestConfig {
  ...
  if (!config.module) {
    ...
    let pkg = cache.pkg || readSync<IPackage>(`${config.rootDir}/package.json`);
...
export function getAliasPaths(
  ...
  let tsConfig = readSync<any>(tsConfigPath);
